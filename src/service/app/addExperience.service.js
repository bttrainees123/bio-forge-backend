...
addExperienceService.add = async (request) => {
    ...
    if (type === 'education') {
        ...
        // No need to check for existing records - multiple records are allowed
    }
...
addExperienceService.update = async (request) => {
    ...
    if (media?.fileMedia) {
        ...
        const tempPath = path.join('public', 'tempUploads', newImage);
            ...
            if (oldImage && oldImage !== newImage) {
                ...
                if (fs.existsSync(oldImagePath)) {
                    fs.unlink(oldImagePath, err => {
                        ...
                        else console.log(`Deleted old image: ${oldImage}`);
                    });
                }
            }
    ...
    if (experienceData.type === 'education') {
        ...
        [
            ...
            'profileHeadline', 'addSkills'
        ].forEach(field => delete body[field]);
    }

    ['userId', 'type', '_id'].forEach(field => delete body[field]);
...
addExperienceService.delete = async (request) => {
    const experience = await addExperienceModel.findOne({ _id: new mongoose.Types.ObjectId(request?.query?._id) });
    ...
    await addExperienceModel.findByIdAndDelete({ _id: new mongoose.Types.ObjectId(request?.query?._id) });
};
